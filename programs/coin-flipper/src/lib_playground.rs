...
pub mod coin_flipper {
    ...
    /// Request VRF randomness (placeholder for Playground)
    pub fn request_vrf(ctx: Context<RequestVrf>) -> Result<()> {
        ...
        // VRF functionality disabled for Playground deployment
        // In production, this would request randomness from Switchboard
        room.vrf_status = VrfStatus::Pending;
        
        msg!("VRF request (simulated) for room {}", room.room_id);
        
        Ok(())
    }

    /// VRF callback (placeholder for Playground)
    pub fn vrf_callback(ctx: Context<VrfCallback>, result: [u8; 32]) -> Result<()> {
        let room = &mut ctx.accounts.game_room;
        
        // VRF functionality disabled for Playground deployment
        // In production, this would receive randomness from Switchboard
        room.vrf_result = Some(result);
        room.vrf_status = VrfStatus::Fulfilled;
        
        msg!("VRF result (simulated) for room {}", room.room_id);
...
// VRF Context structs (simplified for Playground)
#[derive(Accounts)]
...
pub struct VrfCallback<'info> {
    ...
    /// CHECK: VRF oracle account placeholder
...
pub struct CreateMatchedRoom<'info> {
    #[account(
        ...
        payer = matcher,
        space = 8 + 8 + 32 + 32 + 32 + 8 + 1 + 1 + 1 + 8 + 8 + 33 + 1 + 33 + 1,
        seeds = [b"matched_room", room_id.to_le_bytes().as_ref()],
        bump
    )]
    pub game_room: Account<'info, GameRoom>,
